...
describe('click-rally-builder', () => {
  ...
  it('blocked attack (kill block): no player selection, grade # ends rally with point to blocking team', () => {
    let b = createClickRallyBuilder('home', 1);
    b = b.clickZone(1).clickZone(5).skipGrade();
    b = b.clickPlayer(7).skipGrade();
    if (b.step.kind !== 'ATTACK_PLAYER') throw new Error('expected ATTACK_PLAYER');
    b = b.clickPlayer(9).skipZone().clickZone(5).clickGrade('/'); // attack blocked
    expect(b.step.kind).toBe('BLOCK_TOUCH'); // no player selection — straight to touch point
    if (b.step.kind !== 'BLOCK_TOUCH') throw new Error('unreachable');
    ...
    expect(block).toMatchObject({ team: 'home', playerNumber: 0, skill: 'B', startZone: 2, endZone: 9, effect: '#' });
    expect(outcome.pointTeam).toBe('home');
  });
  ...
  it('blocked attack, block does not kill it: rally loops back to the blocking team attacking', () => {
    ...
    expect(b.step.kind).toBe('BLOCK_TOUCH'); // no player selection
    b = b.clickZone(2).clickZone(9).clickGrade('-'); // touched, rally continues
  ...
  it('clickBlockCount is a no-op outside of a block sub-action (no gate to bypass)', () => {
    let b = createClickRallyBuilder('home', 1);
    expect(b.blockersRemaining).toBe(0);
    const unchanged = b.clickBlockCount(2);
    expect(unchanged.step).toEqual(b.step);
    expect(unchanged.blockersRemaining).toBe(0);
  });

  it('block invasion (/): no player selection, point still goes to the attacking team', () => {
    let b = createClickRallyBuilder('home', 1);
    b = b.clickZone(1).clickZone(5).skipGrade();
    b = b.clickPlayer(7).skipGrade();
    if (b.step.kind !== 'ATTACK_PLAYER') throw new Error('expected ATTACK_PLAYER');
    b = b.clickPlayer(9).skipZone().clickZone(5).clickGrade('/');
    expect(b.step.kind).toBe('BLOCK_TOUCH'); // no player selection
    b = b.clickZone(2).clickZone(9).clickGrade('/');
    expect(b.step.kind).toBe('RALLY_DONE');
    if (b.step.kind !== 'RALLY_DONE') throw new Error('unreachable');
    const { outcome } = done(b.step.codeString);
  ...
  it('clickBlockCount is always a no-op — blocker selection removed, blockersRemaining stays 0', () => {
    let b = createClickRallyBuilder('home', 1);
    b = b.clickZone(1).clickZone(5).skipGrade();
    b = b.clickPlayer(7).skipGrade();
    if (b.step.kind !== 'ATTACK_PLAYER') throw new Error('expected ATTACK_PLAYER');
  ...
  it('clickBlockTouch during attack: skips player selection and landing zone, attack finalized as (!), straight to BLOCK_LANDING', () => {
    let b = createClickRallyBuilder('home', 1);
    b = b.clickZone(1).clickZone(5).skipGrade();
    b = b.clickPlayer(7).skipGrade();
    if (b.step.kind !== 'ATTACK_PLAYER') throw new Error('expected ATTACK_PLAYER');
    ...
    b = b.clickBlockTouch(2); // click the block area directly — no player selection
    expect(b.step.kind).toBe('BLOCK_LANDING'); // touch zone captured, straight to landing
    if (b.step.kind !== 'BLOCK_LANDING') throw new Error('unreachable');
    ...
    expect(actions[3]).toMatchObject({ team: 'home', skill: 'B', playerNumber: 0, startZone: 2, endZone: 9, effect: '#' });
    expect(outcome.pointTeam).toBe('home');
  });
